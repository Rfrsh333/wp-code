...
export async function POST(request: NextRequest) {
  ...
  const rateLimitResult = await checkRedisRateLimit(`klant-login:${clientIP}`, klantLoginRateLimit, { failClosed: true });
  ...
  try {
    ...
    const acctLimit = await checkRedisRateLimit(`klant-login-acct:${email.toLowerCase()}`, klantLoginPerAccountRateLimit, { failClosed: true });
