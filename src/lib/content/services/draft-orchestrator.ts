...
import { generateEditorialDraft } from "@/lib/content/services/draft-generation-service";
...
export async function generateDraftsFromTopClusters(limit = 5) {
  ...
  for (const cluster of clusters ?? []) {
    ...
    try {
      ...
      const draft = await generateEditorialDraft(client, {
