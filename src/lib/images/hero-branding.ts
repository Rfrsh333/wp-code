...
function buildOrangeGradient(width: number, height: number): Buffer {
  const svg = `
  <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="orangeGradient" x1="0" y1="1" x2="0" y2="0">
        <stop offset="0%" stop-color="rgba(242,117,1,0.65)" />
        <stop offset="35%" stop-color="rgba(242,117,1,0.35)" />
        <stop offset="65%" stop-color="rgba(242,117,1,0.08)" />
        <stop offset="100%" stop-color="rgba(255,255,255,0)" />
      </linearGradient>
    </defs>
    <rect x="0" y="${Math.round(height * 0.35)}" width="${width}" height="${Math.round(height * 0.65)}" fill="url(#orangeGradient)" />
  </svg>`;
